...
========================
  ...
  Row {IMDB-height=100}
------------------------
  ### Latest Emission Price {.value-box}
  ```{r}
...
### Latest Emission Volume {.value-box}
```{r}
todate1 <- "20230612"
chart1 <- emission.price.down(todate1)
...
### The status of current emission price {.value-box}
```{r}
todate1 <- "20230612"
chart1 <- emission.price.down(todate1)
nowprice <- chart1[1,6]
...
p2 <- ggplot(chart1) +
  ...
  ggtitle("The trading volume of KAU22")
ggplotly(p2)
```
...
### the company emitting highest GHG {.value-box}
...
download.file("https://etrs.gir.go.kr/home/infoOpen/infoOpenList9Excel.do?pagerOffset=0&maxPageItems=10&maxIndexPages=10&searchKey=&searchValue=&menuId=20&condition.plPeriDgr=3&condition.infoOpenYn=Y&condition.pfYy=2022&csvYn=Y", "data.csv")
emission_dt <- read.csv("data.csv", fileEncoding = "EUC-KR")
p <- plot_ly(emission_dt, labels = ~부문, values = ~인증배출량, type = "pie")
p
```
### emission table
...
```{r, include=FALSE}
URL <- "https://scholar.google.com/scholar?as_ylo=2019&q=emission+price&hl=en&as_sdt=0,5"
res <- read_html(URL)
